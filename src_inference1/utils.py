...
class F_th(Metric):
    ...
    def reset(self):
        ...
        self.optimal_th_idx = 0

    def accumulate(self, learn):
        pred, targ = flatten_check(torch.sigmoid(learn.pred.float()), (learn.y > 0.5).float())
    ...
    def value(self):
        self.dices = (
            (1 + self.beta**2) * self.tp / ((1 + self.beta**2) * self.tp + self.beta**2 * self.fn + self.fp + 1e-6)
        )
        self.optimal_th_idx = self.dices.argmax().item()
        return self.dices.max()

    @property
    def optimal_threshold(self):
        return self.ths[self.optimal_th_idx]
